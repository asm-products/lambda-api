...
RSpec.describe User, type: :model do
  describe 'relationships' do
    ...
    it { is_expected.to have_many(:category_gems).dependent(:destroy)
         .inverse_of(:user) }

    it { is_expected.to have_many(:responses).dependent(:nullify)
         .inverse_of(:user) }

    it { is_expected.to have_many(:statistics).dependent(:destroy)
         .inverse_of(:user) }

    it { is_expected.to have_many(:games_won).with_foreign_key('winner_id')
         .class_name('Game').inverse_of(:winner).dependent(:nullify) }

    it { is_expected.to have_many(:games_as_player_1).inverse_of(:player_1)
         .dependent(:nullify).class_name('Game').with_foreign_key('player_1_id') }

    it { is_expected.to have_many(:games_as_player_2).inverse_of(:player_2)
